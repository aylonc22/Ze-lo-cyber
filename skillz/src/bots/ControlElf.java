        ...
        if(!getLocations.oppositePortal(game))
            {
                 ...
                 placePortal(location,closeElf);
                 return true;
            }
        return false;
    }
    // check if possible to build portal and exucute
    public static void placePortal(Location location,Elf chosenElf)
    {
        if(chosenElf.getLocation()==location)
                        chosenElf.buildPortal();
                  else
                        chosenElf.moveTo(location);
